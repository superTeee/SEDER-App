...
struct ExploreView: View {
    ...
    @State private var filterFailure: String?
        ...
        AnyView(
            ...
            .sheet(isPresented: $showFilterSheet) {
                AdvancedFilterSheet(
                    ...
                    catalogOptions:      store.catalogFilters,
                    isLoadingOptions:    store.isLoadingFilters,
                    optionsError:        store.filterError,
                    onRetryOptions:      { Task { await store.loadFilterOptions(force: true) } },
                    onApply: {
                ...
                .task { await store.loadFilterOptions() }
    ...
    var body: some View {
        NavigationStack {
            ...
            .alert("Søket kunne ikke fullføres", isPresented: Binding(
                get: { filterFailure != nil }, set: { if !$0 { filterFailure = nil } }
            )) {
                Button("OK", role: .cancel) { filterFailure = nil }
            } message: { Text(filterFailure ?? "") }
...
>>>>>>> Stashed changes
            ...
            filterFailure = "Kontroller forbindelsen og prøv igjen. Filtrene dine er beholdt."
        }
    }
...
struct AdvancedFilterSheet: View {
    ...
    var catalogOptions = CatalogFilterOptions()
    var isLoadingOptions = false
    var optionsError: String?
    var onRetryOptions: () -> Void = {}
...
>>>>>>> Stashed changes
    ...
    private var chipSelectedBg: Color { Color("Accent").opacity(0.18) }
    // Semantisk accent tilpasser seg lys og mørk modus.
    private var chipStroke: Color { Color("Accent").opacity(0.5) }
        ...
        VStack(spacing: 0) {
            ...
            ZStack {
                ...
                HStack {
                    ...
                    .accessibilityLabel("Lukk avansert søk")
                    .padding(.trailing, 16)
                }
            }
            .frame(height: 54)
            ...
            ScrollView {
                VStack(spacing: 0) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        TextField("Finn et filtervalg", text: $optionSearch)
                            .textFieldStyle(.roundedBorder)
                            .font(.system(size: 15))
                            .autocorrectionDisabled()
                        Text("Flere valg i samme kategori gir flere treff. Ulike kategorier kombineres. Smaksnoter må alle finnes på sigaren.")
                            .font(.system(size: 12))
                            .foregroundColor(Color("TextSecondary"))
                        if isLoadingOptions {
                            ProgressView("Oppdaterer filtervalg …").tint(Color("Accent"))
                        }
                        if let optionsError {
                            Text(optionsError).font(.footnote).foregroundColor(Color("TextSecondary"))
                            Button("Prøv igjen", action: onRetryOptions).tint(Color("Accent"))
                        }
                    }
                    .padding(16)
                    ...
                    sectionDivider()
                    chipSection(title: "TVERRSNITT", options: crossSectionOptions, selection: $crossSection, showAll: $showAllSection)
                    sectionDivider()
                    smokingTimeSection
                    sectionDivider()
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 10) {
                    ...
                    .disabled(isLoadingOptions || (catalogOptions.formats.isEmpty && optionsError != nil))
    ...
    private func chipSection(
        ...
        let matching = options.filter { optionSearch.isEmpty || $0.localizedStandardContains(optionSearch) }
        let visible = showAll.wrappedValue || !optionSearch.isEmpty
            ? matching
            : matching.filter { selection.wrappedValue.contains($0) || matching.prefix(initialCount).contains($0) }
        return VStack(alignment: .leading, spacing: 10) {
            ...
            if matching.isEmpty, !isLoadingOptions {
                Text(optionSearch.isEmpty ? "Ingen registrerte verdier" : "Ingen filtervalg matcher søket")
                    .font(.system(size: 12))
                    .foregroundColor(Color("TextSecondary"))
                    .padding(.horizontal, 16)
            }
            if optionSearch.isEmpty, options.count > initialCount {
                Button(showAll.wrappedValue ? "Vis færre" : "Vis alle (\(options.count))") {
...
struct FilterChip: View {
    ...
    var body: some View {
        Text(title)
            ...
            .overlay(RoundedRectangle(cornerRadius: 16).stroke(isSelected ? Color("Accent") : strokeColor, lineWidth: 1.2))
        ...
        ChipFlowLayout(spacing: 10) {
            ForEach(options, id: \.self) { opt in
                ...
                Button {
                    ...
                    FilterChip(title: opt, subtitle: subtitles[opt], isSelected: isSelected,
                               selectedBg: selectedBg, strokeColor: strokeColor)
                }
                .buttonStyle(.plain)
...
// Two-thumb range slider for 1–5 scale, stepping in 0.1 increments.
...
struct ProfileRangeSlider: View {
    ...
    @State private var lastLow: Double?
    @State private var lastHigh: Double?
            ...
            GeometryReader { geo in
                ...
                ZStack(alignment: .leading) {
                    ...
                    Circle()
                        ...
                        .accessibilityLabel("Minimum \(label.lowercased())")
                        .accessibilityValue(String(format: "%.1f", low))
                        .accessibilityAdjustableAction { direction in
                            switch direction {
                            case .increment: low = min(high, snapped(low + step))
                            case .decrement: low = snapped(low - step)
                            @unknown default: break
                            }
                        }
                        .gesture(DragGesture(minimumDistance: 0)
                            .onChanged { v in
                                if lastLow == nil { lastLow = low }
                                let baseFrac = fraction(lastLow ?? low)
                                ...
                                if cand <= high { low = cand }
                            }
                            .onEnded { _ in lastLow = nil }
                        )
                    ...
                    Circle()
                        ...
                        .offset(x: hiX)
                        .accessibilityElement()
                        .accessibilityLabel("Maksimum \(label.lowercased())")
                        .accessibilityValue(String(format: "%.1f", high))
                        .accessibilityAdjustableAction { direction in
                            switch direction {
                            case .increment: high = snapped(high + step)
                            case .decrement: high = max(low, snapped(high - step))
                            @unknown default: break
                            }
                        }
                        .gesture(DragGesture(minimumDistance: 0)
                            .onChanged { v in
                                if lastHigh == nil { lastHigh = high }
                                let baseFrac = fraction(lastHigh ?? high)
                                let newFrac  = Swift.max(0, Swift.min(1, baseFrac + v.translation.width / trackW))
                                let cand     = snapped(newFrac * (rangeMax - rangeMin) + rangeMin)
                                if cand >= low { high = cand }
                            }
                            .onEnded { _ in lastHigh = nil }
                        )
                }
            }
